...
//   if (!val) return "—";
//   // Si ya viene como dd/mm/yyyy, dejarlo igual
//   if (/^\d{2}\/\d{2}\/\d{4}$/.test(val)) return val;
//   // Si viene como ISO o timestamp, convertirlo
...
const SUPA_KEY    = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InhraGdzdmdveG9idWZidmN1bmdoIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODExODQ0MDQsImV4cCI6MjA5Njc2MDQwNH0.31kUMbV80vYWWgcc3qLAGe6qGOlGVL66CNQWNuAWHZE";
...
function exportarCSV() {
  ...
  const headers = ["OS","Remitente","Dirección Origen","Destino","Estado","Nota","Fecha"];
  const filas   = registros.map(r => {
    const estadoLabel = ESTADOS.find(e => e.val === r.estado)?.label || r.estado;
    return [r.os, r.remitente, r.origen, r.equipos, estadoLabel, r.nota, r.fecha]
      .map(v => `"${(v || "").replace(/"/g, '""')}"`);
